...
import createModalController, { ModalController, RuntimeProject } from '../modalController';
...
describe('ModalController', () => {
        ...
        return [
          {
            ...
            audience: 'Testers',
            slides: [{ image: { src: 'img1.jpg' } }, { image: { src: 'img2.jpg' } }],
          },
        ];
      }
      return [];
    };
    (globalThis as any).Alpine.store.mockImplementation(alpineStoreMock);
  ...
  it('resets state on closeModal', async () => {
    ...
    controller.currentProject = { id: '1', title: 'Test', description: '', audience: '', slides: [] };
  ...
  it('calculates next and previous slide indices', async () => {
    const changeSlideSpy = vi.spyOn(controller, 'changeSlide').mockResolvedValue();
    controller.currentProject = { id: '1', title: 'Test', description: '', audience: '', slides: [{}, {}, {}] };
  ...
  it('returns the current slide', () => {
    controller.currentProject = { id: '1', title: 'Test', description: '', audience: '', slides: [{ task: 'a' }, { task: 'b' }] };
  ...
  it('changeSlide updates slide index and handles transitions', async () => {
    vi.useFakeTimers();
    controller.currentProject = { id: '1', title: 'Test', description: '', audience: '', slides: [{}, { image: { src: 'img2.jpg' } }] };
  ...
  it('initializes and responds to global events', () => {
    ...
    document.dispatchEvent(new CustomEvent(OPEN_MODAL_EVENT, { detail: { projectId: '1' } }));
